...
BEGIN
	...
	ELSEIF(4=_operation) THEN
		BEGIN
			SET @t1 = CONCAT('SELECT u.id, u.name description, IFNULL(pt.description,'''') AS person_type ',
			'FROM user_parent up ',
            ...
            ' LEFT JOIN person_type pt ON p.person_type_id = pt.id AND pt.inactive = 0 AND pt.object_type = ''USU'' ',
			'WHERE ',	
