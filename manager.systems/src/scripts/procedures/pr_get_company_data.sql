	...
	IF (1 = _operation) THEN /** GET DATA INTEGRATION SYSTEMS BRAVO*/
		BEGIN
			...
			WHERE 
				c.id = _company_id;
		END;
	ELSEIF (2 = _operation) THEN /** GET DATA INTEGRATION SYSTEMS BRAVO FOR ALL COMPANYS*/
		BEGIN
			SELECT 
				c.id company_id, c.social_name company_description, c.client_id, ci.integration_system_id, ci.legacy_id
			FROM 
				company c 
				INNER JOIN company_integration_system ci ON c.id = ci.company_id AND ci.integration_system_id = 3 AND ci.inactive = 0
			ORDER BY c.id;
        END;
	END IF; 
END$$

CALL pr_get_company_data(2, 106);
