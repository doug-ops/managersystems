...
BEGIN	
	...
	IF(4=_operation) THEN
		SET @t1 = CONCAT('SELECT m.id, m.document_id, m.document_parent_id, m.document_type, m.credit, m.company_id, m.provider_id, pr.social_name provider_description, m.bank_account_id, m.bank_account_origin_id, bao.description bank_account_origin_description, ', 
                         ...
                         'DATE_FORMAT(m.payment_data,''%d/%m/%Y'') as payment_data, DATE_FORMAT((case when m.document_type <> 4 then m.payment_data else m.payment_expiry_data end),''%d/%m/%Y'') as payment_expiry_data, ', 
						 'm.payment_user, m.inactive, m.user_creation, DATE_FORMAT((case when m.document_type <> 4 then m.payment_data else m.creation_date end),''%d/%m/%Y %H:%i'') as creation_date, m.user_change, ',
                         'DATE_FORMAT((case when m.document_type <> 4 then m.payment_data else m.change_date end),''%d/%m/%Y %H:%i'') as change_date, ', 
...
CALL pr_document_movement_report(4, null, 0, null, null, null, '', '', 1, 1, 0, 0, 1, 0, 1, 0, '2024-10-17 00:00:00', '2024-10-23 09:16:59', 2, null, null, null, 7, 0, 0, '7', null, null, 1, 0, 0, 0  )