...
BEGIN
	...
	BEGIN
		...
		SELECT 'PR FINANCIAL INCOME EXPENSES REPORT', @full_error, 'emarianodeveloper@gmail.com', now(), 0, now(), 0;			
	...
	IF(1 = _operation) THEN /** REPORT SINTETIC */          
		BEGIN
                ...
                (
					SELECT 
						CASE WHEN DAYOFWEEK(days) = 1 THEN yearweek(days -1, 1) ELSE yearweek(days, 1) END days
                    FROM 
              ...
              WHERE
                    ...
                    -- AND m.financial_cost_center_id = (CASE WHEN IFNULL(_financial_cost_center_id,0) = 0 THEN m.financial_cost_center_id ELSE _financial_cost_center_id END)
			  GROUP BY 
				    f.company_id, f.week_year, f.date_from, f.date_to, f.movement_type_id, m.document_parent_id
			  ) x 
			...
			SELECT
                ...
                a.document_id, a.document_parent_id, a.document_type, 
                a.document_note document_note_old,
                CASE WHEN a.document_type in (2) AND document_note LIKE 'TRANSF. FEC. CAIXA >> ORIGEM%' THEN (CASE WHEN a.total < 0 THEN 'DEBITO FEC. CAIXA' ELSE 'CREDITO FEC. CAIXA' END) 
					 WHEN a.document_type in (3) AND document_note LIKE 'DESPESA TRANSF. FEC. CAIXA >> FORNECEDOR:%' THEN UPPER(REPLACE(a.document_note, (CONCAT('FORNECEDOR: ', a.provider_id, ' OBS: ')), ''))
                     WHEN a.document_type in (2) AND document_note LIKE 'TRANFERENCIA: EMP: RGB COFRE CAIXA%' THEN CONCAT(p_origin.id, ' - ', p_origin.social_name)
                     WHEN a.document_type in (3) AND document_note LIKE 'DESPESA: EMP: RGB COFRE CAIXA%' THEN CONCAT(p_origin.id, ' - ', p_origin.social_name)
                ELSE a.document_note END document_note, 
                a.movement_type_id, a.total, 
                CASE 					
					WHEN IFNULL(a.provider_id,0) > 0 AND a.document_type IN (2, 3) AND movement_type_id <> 1 THEN (CASE WHEN p_origin.social_name IS NULL THEN a.document_note ELSE CONCAT(p_origin.id, ' - ', p_origin.social_name) END)
                    WHEN a.movement_type_id in (1) AND document_note LIKE 'TRANSF. FEC. CAIXA >> ORIGEM EMPRESA:%' THEN REPLACE(a.document_note, 'TRANSF. FEC. CAIXA >> ORIGEM EMPRESA:', '')
                    WHEN a.document_type in (2) AND document_note LIKE 'TRANSF. FEC. CAIXA >> ORIGEM EMPRESA:%' THEN REPLACE(a.document_note, 'TRANSF. FEC. CAIXA >> ORIGEM EMPRESA:', '')
                    WHEN a.document_type in (2) AND document_note LIKE 'TRANSF. FEC. CAIXA >> DESTINO EMPRESA:%' THEN a.document_note
                    WHEN a.document_type in (2) AND document_note LIKE 'TRANSF. FEC. CAIXA >> FORNECEDOR:%' THEN ba_origin.description
...
--  pr_financial_incomes_expenses_report(1, '182', '2024-07-01 00:00:00', '2024-07-28 23:59:59', '8', NULL  );