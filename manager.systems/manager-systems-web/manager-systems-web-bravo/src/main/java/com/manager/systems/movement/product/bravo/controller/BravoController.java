...
{	
	...
	public ResponseEntity<String> sincronizeProduct(final HttpServletRequest request) throws Exception
	{
		...
		try
		{
			...
			final CompanyService companyService = new CompanyServiceImpl(connection);
			final List<CompanyIntegrationSystemDTO> companyIntegrationsSystem = companyService.getCompanyIntegrationSystem(0);
			
			final ReportProductDTO reportProduct = new ReportProductDTO();
			reportProduct.setUser(user.getId());
			reportProduct.setInactive(com.manager.systems.common.utils.ConstantDataManager.ZERO_STRING);
			reportProduct.setCompanyId(com.manager.systems.common.utils.ConstantDataManager.ZERO_STRING);
			
			final ProductService productService = new ProductServiceImpl(connection);
			final List<Combobox> allProducts = productService.getAllAutocomplete(reportProduct);
			...
			for (final BillingBravoGamesRoomItemDTO room : rooms) {
				...
				if(StringUtils.isLong(roomIdString) && companyIntegrationsSystem != null) {
					final Optional<CompanyIntegrationSystemDTO> companyData = companyIntegrationsSystem.parallelStream().filter(e -> roomIdString.equalsIgnoreCase(e.getBravoRoomId())).findFirst();
					if(companyData.isPresent()) {
						companyId = companyData.get().getCompanyId();
					}
				}
				...
				for (final BillingBravoGamesJsonItemDTO machine : movements.getRows()) {
					...
					//Discard if product exists
					final Optional<Combobox> productData = allProducts.parallelStream().filter(e -> String.valueOf(machineData.getMachineId()).equalsIgnoreCase(e.getKey())).findFirst();
					...
					product.setAliasProduct(machineData.getMachineDescription().toUpperCase());
					...
					{
						product.setCompanyId(String.valueOf(companyId));
					}
