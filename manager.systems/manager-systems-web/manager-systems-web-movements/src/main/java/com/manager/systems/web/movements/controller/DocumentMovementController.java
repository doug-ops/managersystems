...
public class DocumentMovementController extends BaseController {
	...
	public String open(final HttpServletRequest request, final Model model) throws Exception {
		...
		try {
			...
			final List<Combobox> userChidrensParentFunc = userChidrensParent.parallelStream().filter(x -> x.getKey1()
					.equalsIgnoreCase("FUNCIONARIO")).collect(Collectors.toList());			
			request.setAttribute(ConstantDataManager.PARAMETER_USER_CHIDRENS_PARENT_FUNC, userChidrensParentFunc);
			
			final List<Combobox> userChidrensParentClient = userChidrensParent.parallelStream().filter(x -> x.getKey1()
					.equalsIgnoreCase("CLIENTE")).collect(Collectors.toList());			
			request.setAttribute(ConstantDataManager.PARAMETER_USER_CHIDRENS_PARENT_CLIENT, userChidrensParentClient);
	...
	private DocumentMovementDTO processFilter(final HttpServletRequest request, final Connection connection)
		...
		try {
			...
			final String[] userChildrensParentFuncParameter = request.getParameterValues(ConstantDataManager.PARAMETER_USER_CHIDRENS_PARENT_FUNC_COMBOBOX);
			if (userChildrensParentFuncParameter != null && userChildrensParentFuncParameter.length > 0) {
				for (final String userParent : userChildrensParentFuncParameter) {
					if (userChildrensParentFilters.length() > 0) {
			...
			final String[] userChildrensParentClientParameter = request.getParameterValues(ConstantDataManager.PARAMETER_USER_CHIDRENS_PARENT_CLIENT_COMBOBOX);
			if ((userChildrensParentClientParameter != null && userChildrensParentClientParameter.length > 0)) {
				for (final String userParent : userChildrensParentClientParameter) {
					if (userChildrensParentFilters.length() > 0) {
						userChildrensParentFilters.append(com.manager.systems.common.utils.ConstantDataManager.VIRGULA_STRING);
					}
					userChildrensParentFilters.append(userParent);
			...
			if(userChildrensParentFilters.length() == 0) {
				userChildrensParentFilters.append(String.valueOf(user.getId()));
			}
			documentMovement.setUserChildrensParent(userChildrensParentFilters.toString());
			
			final String financialCostCenterId = request.getParameter(ConstantDataManager.PARAMETER_FILTER_FINANCIAL_COST_CENTER_ID);
