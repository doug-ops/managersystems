...
import com.manager.systems.common.service.cash.impl.CashStatementServiceImpl;
...
@Controller
@RequestMapping(value = ConstantDataManager.CASH_STATEMENT_CONTROLLER)
public class CashStatementController extends BaseController {
	
	private static final Log log = LogFactory.getLog(CashStatementController.class);
	...
	{
		log.info(ConstantDataManager.CASH_STATEMENT_CONTROLLER + ConstantDataManager.COMMON_CONTROLLER_MAINTENANCE_OPEN_METHOD);

		
		String result = ConstantDataManager.CASH_STATEMENT_CONTROLLER_OPEN_METHOD_RESULT;
	...
	{
		log.info(ConstantDataManager.CASH_STATEMENT_CONTROLLER + ConstantDataManager.COMMON_CONTROLLER_MAINTENANCE_FILTER_METHOD);
	...
	{
		final CashStatemenReportDTO report = new CashStatemenReportDTO();
		...
		{	
			...
			final String[] companyParameter = request.getParameterValues(ConstantDataManager.PARAMETER_COMPANY);
			if(companyParameter==null || companyParameter.length==0)
			{
			...
			{
				int count = 0;
				for (final String item : companyParameter) {
					if(count>0)
					{
						companyFilters.append(com.manager.systems.common.utils.ConstantDataManager.VIRGULA_STRING);
					}
					companyFilters.append(item);
					count++;
				}				
			}
			report.setCompanysIds(companyFilters.toString());
				
			final String groupByParameter = request.getParameter(ConstantDataManager.PARAMETER_GROUP_BY);
			if(StringUtils.isLong(groupByParameter))
			{
				report.setGroupBy(Integer.valueOf(groupByParameter));
			} else {
				report.setGroupBy(1); // 1 - week, 2 - month
			}
			...
			report.setDocumentsTypeIds(typeDocumentFilters.toString());			
			...
			service.getCashStatement(report);	
		}
		catch(final Exception ex) 
		{
			throw ex;
	...
	@PostMapping(value=ConstantDataManager.CASH_STATEMENT_CONTROLLER_REPORT_PDF_METHOD)
	public ResponseEntity<byte[]> reportPdf(final HttpServletRequest request) throws Exception
	{
		log.info(ConstantDataManager.CASH_STATEMENT_CONTROLLER + ConstantDataManager.CASH_STATEMENT_CONTROLLER_REPORT_PDF_METHOD);
		...
		{								
			...
			final CashStatemenReportDTO report = this.processFilter(request, connection);
			final CashStatementService providerStatementService = new CashStatementServiceImpl(connection);
