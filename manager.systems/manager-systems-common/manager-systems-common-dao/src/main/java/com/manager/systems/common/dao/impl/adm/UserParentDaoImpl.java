...
public class UserParentDaoImpl implements UserParentDao 
{
	...
	public List<Combobox> getAllParentCombobox(long userId) throws Exception 
	{
		...
		try
		{
	        ...
	        while (resultSet.next()) 
	        {
		        	items.add(new Combobox(resultSet.getString(DatabaseConstants.COLUMN_ID), resultSet.getString(DatabaseConstants.COLUMN_DESCRIPTION), resultSet.getString(DatabaseConstants.COLUMN_PERSON_TYPE)));
	        }	        
		}
		catch(final Exception ex)
		{
